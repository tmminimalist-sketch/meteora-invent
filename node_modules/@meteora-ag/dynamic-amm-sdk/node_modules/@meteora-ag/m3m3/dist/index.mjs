...
var MAX_COMPUTE_UNITS = 14e5;
var STAKE_FOR_FEE_PROGRAM_ID = new PublicKey(
  "FEESngU3neckdwib9X3KWqdL7Mjmqk9XNp3uh5JbP4KP"
);
var DYNAMIC_AMM_PROGRAM_ID = new PublicKey(
  "Eo7WjKq67rjJQSZxS6z3YkapzY3eMj6Xy8X5EQVn5UaB"
);
var DYNAMIC_VAULT_PROGRAM_ID = new PublicKey(
  "24Uqj9JCLxUeoC3hGfh5W3s9FM9uCHDS2SG3LYwBpyTi"
);
var FULL_BALANCE_LIST_HARD_LIMIT = new BN(1e4);
var U64_MAX = new BN("18446744073709551615");
var VAULT_WITH_NON_PDA_BASED_LP_MINTS = /* @__PURE__ */ new Map([
  [
...
var MIN_LOCK_ESCROW_CLAIM_FEE_DURATION = new BN2(300);
...
var LOCKED_PROFIT_DEGRADATION_DENOMINATOR = new BN3(1e12);
...
function getTokenBalances(currentTime, aVault, bVault, aVaultLp, bVaultLp, aVaultLpMint, bVaultLpMint) {
  const tokenAAmount = getAmountByShare(
    currentTime,
    aVault,
    new BN4(aVaultLp.amount.toString()),
    new BN4(aVaultLpMint.supply.toString())
  );
  const tokenBAmount = getAmountByShare(
    currentTime,
    bVault,
    new BN4(bVaultLp.amount.toString()),
    new BN4(bVaultLpMint.supply.toString())
  );
...
function getVirtualPrice(tokenAAmount, tokenBAmount, lpSupply) {
  ...
  const d = new BN4(new Decimal(k.toString()).sqrt().floor().toString());
...
function getLockedEscrowPendingFee(currentTime, feeVault, lockEscrow, aVault, bVault, aVaultLp, bVaultLp, aVaultLpMint, bVaultLpMint, poolLpMint) {
  ...
  const currentLpPerToken = getVirtualPrice(
    tokenAAmount,
    tokenBAmount,
    new BN4(poolLpMint.supply.toString())
  );
  const newFee = currentLpPerToken.gt(lockEscrow.lpPerToken) ? lockEscrow.totalLockedAmount.mul(currentLpPerToken.sub(lockEscrow.lpPerToken)).div(currentLpPerToken) : new BN4(0);
  if (newFee.isZero()) {
    return [new BN4(0), new BN4(0)];
  }
  const aVaultLpToBurn = newFee.mul(new BN4(aVaultLp.amount.toString())).div(new BN4(poolLpMint.supply.toString()));
  const bVaultLpToBurn = newFee.mul(new BN4(bVaultLp.amount.toString())).div(new BN4(poolLpMint.supply.toString()));
...
var deriveLpMint = (poolAddress, programId) => {
  return PublicKey2.findProgramAddressSync(
    ...
    new PublicKey2(programId || DYNAMIC_AMM_PROGRAM_ID)
  )[0];
};
...
function createStakeFeeProgram(connection, programId) {
  ...
  return new Program2(IDL, programId, provider);
}
function createDynamicAmmProgram(connection, programId) {
  ...
  return new Program2(IDL2, programId, provider);
}
function createDynamicVaultProgram(connection, programId) {
  ...
  return new Program2(IDL3, programId, provider);
}
...
function findLargestStakerNotInTopListFromFullBalanceList(lookupNumber, fullBalanceListState) {
  ...
  const fullBalanceListWithoutTopStakers = [...fullBalanceListState.stakers].map((s, idx) => [idx, s]).filter(
    ([_idx, s]) => s.owner != PublicKey4.default && !Boolean(s.isInTopList)
  );
...
function getTopStakerListStateEntryStakeAmount(topStakerListState) {
  let smallestStakeAmount = new BN5("18446744073709551615");
  ...
  return smallestStakeAmount.add(new BN5(1));
}
function calculateFeeFarmPerformance(accountStatesT0, accountStatesT1, tokenAUIMultiplier, tokenBUIMultiplier, stakeTokenUIMultiplier, tokenAUsdRate, tokenBUsdRate, stakeTokenUsdRate) {
  const secondsElapsed = new Decimal2(
  ...
  const feeAAmountUi = new Decimal2(feeAAmount.toString()).div(
    new Decimal2(tokenAUIMultiplier)
  );
  const feeBAmountUi = new Decimal2(feeBAmount.toString()).div(
    new Decimal2(tokenBUIMultiplier)
  );
  const feeUsd = feeAAmountUi.mul(tokenAUsdRate).add(feeBAmountUi.mul(tokenBUsdRate));
  const usdPerDay = feeUsd.div(secondsElapsed).mul(new Decimal2(86400));
  const annualizedFeeUsd = usdPerDay.mul(new Decimal2(365));
  const effectiveStakeAmountUi = new Decimal2(
  ...
  const effectiveRate = new Decimal2(1).add(nominalRate.div(new Decimal2(365))).pow(new Decimal2(365)).sub(new Decimal2(1));
  return {
    usdPerDay,
    apr: nominalRate.mul(new Decimal2(100)),
    apy: effectiveRate.mul(new Decimal2(100))
  };
}
function parseFeeVaultInfo(accountStatesT0, accountStatesT1, tokenAUsdRate, tokenBUsdRate) {
  ...
  const totalStakedAmount = new Decimal2(
  ...
  const entryStakeAmount = new Decimal2(
  ...
  const secondsElapsedSinceCreated = new Decimal2(
    clock.unixTimestamp.sub(feeVault.createdAt).toString()
  );
  const rawContext = {
    totalRewardA: totalFeeAAmount,
    totalRewardB: totalFeeBAmount,
    dailyRewardA: totalFeeAAmount.mul(new Decimal2(86400)).div(secondsElapsedSinceCreated),
    dailyRewardB: totalFeeBAmount.mul(new Decimal2(86400)).div(secondsElapsedSinceCreated),
    totalStakers: new Decimal2(
...
function parseTopStakerListState(topStakerInfo, topStakerListState, stakeTokenDecimal, feeVaultUsdPerDay) {
  const stakeTokenUiMultiplier = new Decimal2(
    Math.floor(10 ** stakeTokenDecimal)
  );
  const effectiveStakeAmount = new Decimal2(
  ...
  return orderedTopStakers.map((staker, idx) => {
    const rank = idx + 1;
    const totalStaked = new Decimal2(staker.stakeAmount.toString()).div(
      stakeTokenUiMultiplier
    );
...
function getStakeEscrowEarningPerDay(stakeEscrow, topStakerInfo, feeVaultUsdPerDay) {
  if (Boolean(stakeEscrow.inTopList)) {
    return new Decimal2(stakeEscrow.stakeAmount.toString()).mul(feeVaultUsdPerDay).div(new Decimal2(topStakerInfo.effectiveStakeAmount.toString()));
  }
...
function getStakeEscrowEarningPerDayAfterUnstake(stakeEscrow, topStakerInfo, unstakeAmount, feeVaultUsdPerDay) {
  ...
  if (Boolean(stakeEscrow.inTopList)) {
    return new Decimal2(newStakeAmount.toString()).mul(feeVaultUsdPerDay).div(new Decimal2(newEffectiveStakeAmount.toString()));
  }
  return new Decimal2(0);
}
...
var unwrapSOLInstruction = async (owner) => {
  const wSolATAAccount = getAssociatedTokenAddressSync2(NATIVE_MINT, owner);
  if (wSolATAAccount) {
...
var getSimulationComputeUnits = async (connection, instructions, payer, lookupTables, commitment = "confirmed") => {
  const testInstructions = [
    ...
    ComputeBudgetProgram2.setComputeUnitLimit({ units: MAX_COMPUTE_UNITS }),
    ...instructions
  ];
...
var StakeForFee2 = class _StakeForFee {
  ...
  static async create(connection, pool, opt) {
    ...
    const escrowVaultKey = getAssociatedTokenAddressSync3(
  ...
  static async fetchAccountStates(connection, feeVaultKey, topStakerListKey, fullBalanceListKey, pool, opt) {
    ...
    const aVaultLpState = AccountLayout.decode(
      new Uint8Array(aVaultLpAccount.data)
    );
    const bVaultLpState = AccountLayout.decode(
      new Uint8Array(bVaultLpAccount.data)
    );
    const tokenAMintState = MintLayout.decode(
      new Uint8Array(tokenAMintAccount.data)
    );
    const tokenBMintState = MintLayout.decode(
      new Uint8Array(tokenBMintAccount.data)
    );
    const poolLpMintState = MintLayout.decode(
    ...
    const aVaultLpMintState = MintLayout.decode(
      new Uint8Array(aVaultLpMintAccount.data)
    );
    const bVaultLpMintState = MintLayout.decode(
  ...
  static async createFeeVault(connection, pool, stakeMint, payer, param, opt) {
    ...
    const feeVaultKey = deriveFeeVault(pool, stakeForFeeProgram.programId);
    const stakeTokenVaultKey = getAssociatedTokenAddressSync3(
      stakeMint,
      feeVaultKey,
      true
    );
    const quoteTokenVaultKey = getAssociatedTokenAddressSync3(
      quoteMint,
      feeVaultKey,
      true
    );
  ...
  async stake(maxAmount, owner, replaceableTopStakerCount = 2) {
    ...
    const userStakeTokenKey = getAssociatedTokenAddressSync3(
      this.accountStates.feeVault.stakeMint,
      owner
    );
    const remainingAccounts = [];
   ...
   */
  static async getAllStakedVaultByUser(connection, owner, opt) {
    const stakeForFeeProgram = createStakeFeeProgram(
      connection,
    ...
    const unclaimFeeMap = /* @__PURE__ */ new Map();
    const stakeEscrowMap = /* @__PURE__ */ new Map();
    const poolsToFetch = [];
    const lockEscrowToFetch = [];
    ...
    const accounts = (await Promise.all(
      chunkedAccountToFetch.map(
        (accounts2) => connection.getMultipleAccountsInfo(accounts2)
      )
    )).flat();
    ...
    const poolAccountsMap = poolAccountsBuffer.map(
      (account) => dynamicAmmProgram.coder.accounts.decode("pool", account.data)
    ).reduce((acc, pool, index) => {
      acc.set(poolsToFetch[index], pool);
      return acc;
    }, /* @__PURE__ */ new Map());
    const lockEscrowAccounts = lockEscrowAccountsBuffer.map(
      (account) => dynamicAmmProgram.coder.accounts.decode("lockEscrow", account.data)
    ).reduce((acc, lockEscrow, index) => {
    ...
    const poolAccountsToFetch = chunks(
      Array.from(poolAccountsMap.values()).flatMap(
        ({ aVault, aVaultLp, bVault, bVaultLp, lpMint }) => {
          const aVaultLpMint = VAULT_WITH_NON_PDA_BASED_LP_MINTS.get(aVault.toBase58()) ?? PublicKey7.findProgramAddressSync(
            [Buffer.from("lp_mint"), aVault.toBuffer()],
            dynamicVaultProgram.programId
          )[0];
          const bVaultLpMint = VAULT_WITH_NON_PDA_BASED_LP_MINTS.get(bVault.toBase58()) ?? PublicKey7.findProgramAddressSync(
    ...
    const poolLpAccounts = (await Promise.all(
      poolAccountsToFetch.map(
        (accounts2) => connection.getMultipleAccountsInfo(accounts2)
      )
    )).flat();
    const poolInfoAccountsMap = /* @__PURE__ */ new Map();
    for (let i = 0; i < poolLpAccounts.length; i += 7) {
      ...
      const aVault = dynamicVaultProgram.coder.accounts.decode(
        "vault",
        aVaultAccountBuffer.data
      );
      const bVault = dynamicVaultProgram.coder.accounts.decode(
        "vault",
        bVaultAccountBuffer.data
      );
      const aVaultLp = AccountLayout.decode(
        new Uint8Array(aVaultLpAccountBuffer.data)
      );
      const bVaultLp = AccountLayout.decode(
        new Uint8Array(bVaultLpAccountBuffer.data)
      );
      const poolLpMint = MintLayout.decode(
        new Uint8Array(lpMintAccountBuffer.data)
      );
      const aVaultLpMint = MintLayout.decode(
        new Uint8Array(aVaultLpMintAccountBuffer.data)
      );
      const bVaultLpMint = MintLayout.decode(
    ...
    poolsToFetch.forEach((pool) => {
      ...
      const [releasedFeeA, releasedFeeB] = _StakeForFee.getFarmReleasedFees({
        feeVault,
        clock: clockState,
        lockEscrow,
      ...
      const newFeeAPerLiquidity = releasedFeeA.isNeg() || effectiveStakeAmount.isZero() ? new BN6(0) : releasedFeeA.shln(64).div(effectiveStakeAmount);
      const newFeeBPerLiquidity = releasedFeeB.isNeg() || effectiveStakeAmount.isZero() ? new BN6(0) : releasedFeeB.shln(64).div(effectiveStakeAmount);
      const newCumulativeFeeAPerLiquidity = feeVault.topStakerInfo.cumulativeFeeAPerLiquidity.add(
        newFeeAPerLiquidity
      );
      const newCumulativeFeeBPerLiquidity = feeVault.topStakerInfo.cumulativeFeeBPerLiquidity.add(
        newFeeBPerLiquidity
      );
      const newFeeA = newCumulativeFeeAPerLiquidity.sub(stakeEscrow.feeAPerLiquidityCheckpoint).mul(stakeEscrow.stakeAmount).ushrn(64);
      const newFeeB = newCumulativeFeeBPerLiquidity.sub(stakeEscrow.feeBPerLiquidityCheckpoint).mul(stakeEscrow.stakeAmount).ushrn(64);
...
export {
  ...
  getLockedEscrowPendingFee,
  getOrCreateATAInstruction,
  getOrCreateStakeEscrowInstruction,
  getStakeEscrowEarningPerDay,
  getStakeEscrowEarningPerDayAfterUnstake,
  getTokenBalances,
  getTopStakerListStateEntryStakeAmount,
  getVirtualPrice,
  parseFeeVaultInfo,
  parseTopStakerListState
};
