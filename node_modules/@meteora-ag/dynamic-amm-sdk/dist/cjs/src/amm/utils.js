...
const vault_sdk_1 = require("@meteora-ag/vault-sdk");
...
const unwrapSOLInstruction = async (owner, receiver) => {
    const wSolATAAccount = await (0, exports.getAssociatedTokenAccount)(spl_token_1.NATIVE_MINT, owner);
    if (wSolATAAccount) {
        const closedWrappedSolInstruction = (0, spl_token_1.createCloseAccountInstruction)(wSolATAAccount, receiver ? receiver : owner, owner, []);
...
function derivePoolAddress(connection, tokenA, tokenB, tokenADecimal, tokenBDecimal, isStable, tradeFeeBps, opt) {
    const { ammProgram } = (0, exports.createProgram)(connection, opt?.programId);
 ...
 */
async function checkPoolExists(connection, mintA, mintB, mintADecimal, mintBDecimal, isStable, tradeFeeBps, opt) {
    const { ammProgram } = (0, exports.createProgram)(connection, opt?.programId);
...
function generateCurveType(mintADecimal, mintBDecimal, isStable) {
    return isStable
        ? {
            stable: {
                amp: constants_1.PERMISSIONLESS_AMP,
                tokenMultiplier: (0, exports.computeTokenMultiplier)(mintADecimal, mintBDecimal),
