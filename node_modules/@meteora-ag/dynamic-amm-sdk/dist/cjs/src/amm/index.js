...
const vault_sdk_1 = __importStar(require("@meteora-ag/vault-sdk"));
...
class AmmImpl {
    ...
    static async createPermissionlessConstantProductPoolWithConfig2(connection, payer, tokenAMint, tokenBMint, tokenAAmount, tokenBAmount, config, opt) {
        ...
        if (feeWrapperLockAmount.gt(new anchor_1.BN(0))) {
            ...
            const createFeeVaultIxs = await m3m3_1.default.createFeeVaultInstructions(connection, poolPubkey, payer, tokenAMint, tokenBMint, {
                unstakeLockDuration: opt?.stakeLiquidity?.param.unstakeLockDuration || new anchor_1.BN(0),
                secondsToFullUnlock: opt?.stakeLiquidity?.param.secondsToFullUnlock || new anchor_1.BN(0),
                topListLength: opt?.stakeLiquidity?.param.topListLength || 0,
                startFeeDistributeTimestamp: opt?.stakeLiquidity?.param.startFeeDistributeTimestamp || new anchor_1.BN(0),
    ...
    static async createPermissionlessPool(connection, payer, mintA, mintB, tokenAAmount, tokenBAmount, isStable, tradeFeeBps, opt) {
        const { vaultProgram, ammProgram } = (0, utils_1.createProgram)(connection, opt?.programId);
        const [tokenMintA, tokenMintB] = await Promise.all([mintA, mintB].map((m) => (0, spl_token_1.getMint)(connection, m)));
        const curveType = (0, utils_1.generateCurveType)(tokenMintA.decimals, tokenMintB.decimals, isStable);
        const [{ vaultPda: aVault, tokenVaultPda: aTokenVault, lpMintPda: aLpMintPda }, { vaultPda: bVault, tokenVaultPda: bTokenVault, lpMintPda: bLpMintPda },] = [(0, vault_sdk_1.getVaultPdas)(mintA, vaultProgram.programId), (0, vault_sdk_1.getVaultPdas)(mintB, vaultProgram.programId)];
        const [aVaultAccount, bVaultAccount] = await Promise.all([
            vaultProgram.account.vault.fetchNullable(aVault),
            vaultProgram.account.vault.fetchNullable(bVault),
        ]);
        ...
        if (!aVaultAccount) {
            const createVaultAIx = await vault_sdk_1.default.createPermissionlessVaultInstruction(connection, payer, mintA);
            createVaultAIx && preInstructions.push(createVaultAIx);
        }
        else {
            aVaultLpMint = aVaultAccount.lpMint; // Old vault doesn't have lp mint pda
        ...
        if (!bVaultAccount) {
            const createVaultBIx = await vault_sdk_1.default.createPermissionlessVaultInstruction(connection, payer, mintB);
            createVaultBIx && preInstructions.push(createVaultBIx);
        }
        else {
            bVaultLpMint = bVaultAccount.lpMint; // Old vault doesn't have lp mint pda
        }
        const poolPubkey = (0, utils_1.derivePoolAddress)(connection, tokenMintA.address, tokenMintB.address, tokenMintA.decimals, tokenMintB.decimals, isStable, tradeFeeBps, {
        ...
        const [[payerTokenA, createPayerTokenAIx], [payerTokenB, createPayerTokenBIx]] = await Promise.all([
            (0, utils_1.getOrCreateATAInstruction)(mintA, payer, connection),
            (0, utils_1.getOrCreateATAInstruction)(mintB, payer, connection),
        ]);
        if (!opt?.skipAta) {
            createPayerTokenAIx && preInstructions.push(createPayerTokenAIx);
        }
        createPayerTokenBIx && preInstructions.push(createPayerTokenBIx);
        const [[protocolTokenAFee], [protocolTokenBFee]] = [
            web3_js_1.PublicKey.findProgramAddressSync([Buffer.from(constants_1.SEEDS.FEE), mintA.toBuffer(), poolPubkey.toBuffer()], ammProgram.programId),
            web3_js_1.PublicKey.findProgramAddressSync([Buffer.from(constants_1.SEEDS.FEE), mintB.toBuffer(), poolPubkey.toBuffer()], ammProgram.programId),
        ];
        const [lpMint] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from(constants_1.SEEDS.LP_MINT), poolPubkey.toBuffer()], ammProgram.programId);
        const payerPoolLp = await (0, utils_1.getAssociatedTokenAccount)(lpMint, payer);
        if (mintA.equals(spl_token_1.NATIVE_MINT)) {
            preInstructions = preInstructions.concat((0, utils_1.wrapSOLInstruction)(payer, payerTokenA, BigInt(tokenAAmount.toString())));
        }
        if (mintB.equals(spl_token_1.NATIVE_MINT)) {
            preInstructions = preInstructions.concat((0, utils_1.wrapSOLInstruction)(payer, payerTokenB, BigInt(tokenBAmount.toString())));
        }
        const [mintMetadata, _mintMetadataBump] = (0, utils_1.deriveMintMetadata)(lpMint);
        const createPermissionlessPoolTx = await ammProgram.methods
    ...
    async swap(owner, inTokenMint, inAmountLamport, outAmountLamport, referralOwner) {
        ...
        if (spl_token_1.NATIVE_MINT.equals(destinationToken)) {
            const unwrapSOLIx = await (0, utils_1.unwrapSOLInstruction)(owner, owner);
    ...
    async deposit(owner, tokenAInAmount, tokenBInAmount, poolTokenAmount) {
        ...
        if ([this.tokenAMint.address.toBase58(), this.tokenBMint.address.toBase58()].includes(spl_token_1.NATIVE_MINT.toBase58())) {
            const closeWrappedSOLIx = await (0, utils_1.unwrapSOLInstruction)(owner, owner);
    ...
    async claimWithQuoteMintSol({ owner, payer, receiver, tempWSolAcc, lockEscrowPK, }) {
        const preInstructions = [];
        const postInstructions = [];
        const isTokenAMintNative = this.poolState.tokenAMint.toBase58() === spl_token_1.NATIVE_MINT.toBase58();
        const isTokenBMintNative = this.poolState.tokenBMint.toBase58() === spl_token_1.NATIVE_MINT.toBase58();
        const [[userAta, createUserAtaIx], [escrowAta, createEscrowAtaIx], [tokenAAta, createTokenAAtaIx], [tokenBAta, createTokenBAtaIx],] = await Promise.all([
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.lpMint, owner, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.lpMint, lockEscrowPK, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.tokenAMint, isTokenAMintNative ? tempWSolAcc : receiver, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.tokenBMint, isTokenBMintNative ? tempWSolAcc : receiver, this.program.provider.connection, payer),
        ]);
        createUserAtaIx && preInstructions.push(createUserAtaIx);
        createEscrowAtaIx && preInstructions.push(createEscrowAtaIx);
        createTokenAAtaIx && preInstructions.push(createTokenAAtaIx);
        createTokenBAtaIx && preInstructions.push(createTokenBAtaIx);
        const closeWrappedSOLIx = await (0, utils_1.unwrapSOLInstruction)(tempWSolAcc, receiver);
        closeWrappedSOLIx && postInstructions.push(closeWrappedSOLIx);
        const accounts = {
            pool: this.address,
            lockEscrow: lockEscrowPK,
    ...
    async claimWithQuoteMintNotSol({ owner, payer, receiver, lockEscrowPK, }) {
        const preInstructions = [];
        const [[userAta, createUserAtaIx], [escrowAta, createEscrowAtaIx], [tokenAAta, createTokenAAtaIx], [tokenBAta, createTokenBAtaIx],] = await Promise.all([
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.lpMint, owner, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.lpMint, lockEscrowPK, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.tokenAMint, receiver, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.tokenBMint, receiver, this.program.provider.connection, payer),
        ]);
        createUserAtaIx && preInstructions.push(createUserAtaIx);
        createEscrowAtaIx && preInstructions.push(createEscrowAtaIx);
        createTokenAAtaIx && preInstructions.push(createTokenAAtaIx);
    ...
    async claimLockFee(owner, maxAmount, payer, receiver, tempWSolAcc) {
        const [lockEscrowPK] = (0, utils_1.deriveLockEscrowPda)(this.address, owner, this.program.programId);
        const isTokenAOrBNative = this.poolState.tokenAMint.toBase58() === spl_token_1.NATIVE_MINT.toBase58() ||
            this.poolState.tokenBMint.toBase58() === spl_token_1.NATIVE_MINT.toBase58();
        let tx;
     ...
     */
    async moveLockedLP(owner, newOwner, maxAmount, payer) {
        payer = payer ?? owner;
        const [ownerLockEscrowPK] = (0, utils_1.deriveLockEscrowPda)(this.address, owner, this.program.programId);
        const [newOwnerLockEscrowPK] = (0, utils_1.deriveLockEscrowPda)(this.address, newOwner, this.program.programId);
        const preInstructions = [];
        ...
        const fromEscrowVault = (0, spl_token_1.getAssociatedTokenAddressSync)(this.poolState.lpMint, ownerLockEscrowPK);
        const [toEscrowVault, createToEscrowVaultIx] = await (0, utils_1.getOrCreateATAInstruction)(this.poolState.lpMint, newOwnerLockEscrowPK, this.program.provider.connection, payer);
