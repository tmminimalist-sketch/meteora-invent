...
import { StakeForFee as StakeForFeeIdl } from '@meteora-ag/m3m3';
...
export declare const createProgram: (connection: Connection, programId?: string) => {
    ...
    stakeForFeeProgram: Program<StakeForFeeIdl>;
};
/**
...
export declare const unwrapSOLInstruction: (owner: PublicKey, receiver: PublicKey) => Promise<TransactionInstruction | null>;
...
export declare function deriveCustomizablePermissionlessConstantProductPoolAddress(tokenA: PublicKey, tokenB: PublicKey, programId: PublicKey): PublicKey;
...
export declare function derivePoolAddress(connection: Connection, tokenA: PublicKey, tokenB: PublicKey, tokenADecimal: number, tokenBDecimal: number, isStable: boolean, tradeFeeBps: BN, opt?: {
    programId?: string;
}): PublicKey;
/**
 ...
 */
export declare function checkPoolExists(connection: Connection, mintA: PublicKey, mintB: PublicKey, mintADecimal: number, mintBDecimal: number, isStable: boolean, tradeFeeBps: BN, opt?: {
    programId: string;
...
export declare function generateCurveType(mintADecimal: number, mintBDecimal: number, isStable: boolean): {
    stable: {
        amp: BN;
...
export declare function createMint(connection: Connection, mintAccount: Keypair, payer: PublicKey, assetData: DataV2, mintAuthority: PublicKey, freezeAuthority: PublicKey | null, decimals: number, programId: PublicKey): Promise<{
    tx: Transaction;
    mintAccount: Keypair;
}>;
export declare const calculateLockAmounts: (amount: BN, feeWrapperRatio?: Decimal) => {
    feeWrapperLockAmount: BN;
    userLockAmount: BN;
};
export declare function createTransactions(connection: Connection, ixs: Array<Transaction | TransactionInstruction | (Transaction | TransactionInstruction)[]>, payer: PublicKey): Promise<Transaction[]>;
